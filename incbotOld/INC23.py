...
# BROADCAST_GROUPS = [-1002580433661, -1002631818202, -4954181055]  # Обновите старый ID
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(word in text for word in priority_words):
        ...
        if ("поднят до" in text or "повышен до" in text) and ("Высокий" in text or "Критичный" in text):
            print(f"Приоритет инцидента повышен.")
            await context.bot.send_message(chat_id=chat_id, text=f"Приоритет инцидента '{key}' повышен.")
            ...
            await context.bot.send_message(chat_id=chat_id, text=f"Приоритет инцидента '{key}' понижен.")
            for job_id in incident["jobs"]:
                try: scheduler.remove_job(job_id)
                except: pass
            incident["jobs"] = []
...
def notify_3_hours_later(app, chat_id, incident_id, loop):
    ...
    asyncio.run_coroutine_threadsafe(
        app.bot.send_message(chat_id=chat_id, text="С момента оповещения прошло 3 часа! Проверьте статус и вновь оповестите. @PR @Ruslank1111"),
        loop
    )
