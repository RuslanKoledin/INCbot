...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Закрытие инцидента ---
    if reply_to and any(word in text.lower() for word in ["заработал", "устранено", "решено", "локализован", "восстановлен"]):
        ...
        if incident:
            ...
            # Добавляем ссылку на Jira (пример)
            jira_link = f"https://jiraportal.cbk.kg/projects/ITSMJIRA/queues/issue/ITSMJIRA-{key}"
    ...
    # --- Изменение приоритета ---
        ...
        if reply_to:
            ...
            # Убираем айди группы из incident_id для сообщения
            ...
            if ("поднят до" in text or "повышен до" in text) and ("Высокий" in text or "Наивысший" in text):
                ...
                remain_50 = max(datetime.timedelta(seconds=10) - elapsed, datetime.timedelta())
                remain_60 = max(datetime.timedelta(seconds=20) - elapsed, datetime.timedelta())
