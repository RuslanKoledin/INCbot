...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Закрытие инцидента ---
    if reply_to and any(word in text.lower() for word in ["заработал", "устранено", "решено","восстановлено","локализован"]):
        ...
        if reply_to:
            ...
            if "поднят до" in text and ("Высокий" in text or "Наивысший" in text):
                ...
                if not incident["jobs"]:
                    now = datetime.datetime.now(tz=ZoneInfo("Asia/Bishkek"))
                    start_time = incident["time"]  # время создания инцидента
                    delta = now - start_time
                    seconds_passed = delta.total_seconds()

                    delay_50 = 30 - seconds_passed
                    delay_60 = 60 - seconds_passed

                    # Если время уже прошло, то не ставим напоминания
                    if delay_50 <= 0:
                        print("⏳ Время для 50-секундного напоминания уже прошло — не ставим.")
                        delay_50 = None
                    if delay_60 <= 0:
                        print("⏳ Время для 60-секундного напоминания уже прошло — не ставим.")
                        delay_60 = None

                    if delay_50 is not None:
                        job_50 = scheduler.add_job(
                            notify_50_minutes,
                            trigger='date',
                            run_date=now + datetime.timedelta(seconds=delay_50),
                            args=[context.application, chat_id, incident_id],
                            id=f"{incident_id}_50"
                        )
                        incident["jobs"].append(job_50.id)

                    if delay_60 is not None:
                        job_60 = scheduler.add_job(
                            notify_60_minutes,
                            trigger='date',
                            run_date=now + datetime.timedelta(seconds=delay_60),
                            args=[context.application, chat_id, incident_id],
                            id=f"{incident_id}_60"
                        )
                        incident["jobs"].append(job_60.id)

                    if delay_50 is not None or delay_60 is not None:
