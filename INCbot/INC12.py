...
GROUP_NAMES = {
    -1002783899407: "Мбизнес",  # главный чат для уведомлений 50/60 минут
    -1002591060921: "Группа 1", # test2
    -1002631818202: "Группа 2", # test1
}

# Список групп для рассылки инцидентов и сообщений о решении (без главного чата)
BROADCAST_GROUPS = [-1002591060921, -1002631818202]

# Главный чат, куда идут напоминания 50/60 минут
MAIN_NOTIFY_CHAT = -1002783899407
...
async def send_notify_50(application):
    print(f"Отправляю уведомление 50 минут в главный чат {MAIN_NOTIFY_CHAT}")
    await application.bot.send_message(
        chat_id=MAIN_NOTIFY_CHAT,
        text="С момента создания прошло 50 минут, через 10 минут надо оповестить. @PR @"
    )

async def send_notify_60(application):
    print(f"Отправляю уведомление 60 минут в главный чат {MAIN_NOTIFY_CHAT}")
    await application.bot.send_message(
        chat_id=MAIN_NOTIFY_CHAT,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reply_to and any(word in text.lower() for word in ["заработал", "устранено", "решено"]):
        ...
        if incident:
            ...
            # И пересылаем в группы рассылки (без главного чата)
    ...
    if "Инцидент:" in text and "Время выявления инцидента:" in text:
        ...
        # Рассылаем текст инцидента в группы рассылки (test1 и test2)
        for group_id in BROADCAST_GROUPS:
        ...
        if not is_priority_low(text):
            job_50 = scheduler.add_job(
                notify_50_minutes,
                trigger='date',
                run_date=now + datetime.timedelta(seconds=30),  # для теста 30 секунд; замените на minutes=50 в реале
                args=[context.application, incident_id],
                id=f"{incident_id}_50"
            )
            job_60 = scheduler.add_job(
                notify_60_minutes,
                trigger='date',
                run_date=now + datetime.timedelta(seconds=60),  # для теста 60 секунд; замените на minutes=60 в реале
                args=[context.application, incident_id],
    ...
    elif any(phrase in text for phrase in ["Приоритет инцидента поднят до", "Приоритет инцидента повышен до", "Приоритет инцидента понижен до"]):
        if reply_to:
            replied_text = reply_to.text
            ...
            if ("поднят до" in text or "повышен до" in text) and ("Высокий" in text or "Наивысший" in text):
                ...
                # Отправляем уведомление о повышении во группы рассылки (test1 и test2)
                for group_id in BROADCAST_GROUPS:
                ...
                if not incident["jobs"]:
                    ...
                    job_50 = scheduler.add_job(
                        notify_50_minutes,
                        trigger='date',
                        run_date=now + datetime.timedelta(seconds=10),  # для теста 10 секунд; замените на minutes=50
                        args=[context.application, incident_id],
                        id=f"{incident_id}_50"
                    )
                    job_60 = scheduler.add_job(
                        notify_60_minutes,
                        trigger='date',
                        run_date=now + datetime.timedelta(seconds=25),  # для теста 25 секунд; замените на minutes=60
                        args=[context.application, incident_id],
                        id=f"{incident_id}_60"
                    )
                    ...
                    # Проверяем, если прошло уже много — отправляем уведомления немедленно
                    if elapsed >= datetime.timedelta(seconds=20):  # для теста
                        print(f"⚠️ Инцидент {incident_id} - прошло более 20 секунд. Немедленно отправляю уведомление 50 мин.")
                        await send_notify_50(context.application)

                    if elapsed >= datetime.timedelta(seconds=30):  # для теста
                        print(f"⚠️ Инцидент {incident_id} - прошло более 30 секунд. Немедленно отправляю уведомление 60 мин.")
                        await send_notify_60(context.application)
            ...
            # Понижение приоритета — удаляем напоминания
                ...
                # Отправляем уведомление о понижении приоритета в группы рассылки
                for group_id in BROADCAST_GROUPS:
                    await context.bot.send_message(
                        chat_id=group_id,
...
def notify_50_minutes(application, incident_id):
    global event_loop
    ...
    asyncio.run_coroutine_threadsafe(
        send_notify_50(application),
        event_loop
    )

def notify_60_minutes(application, incident_id):
    global event_loop
    if incident_id not in incidents:
