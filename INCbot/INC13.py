    ...
    # --- Изменение приоритета ---
        ...
        if reply_to:
            ...
            if ("поднят до" in text or "повышен до" in text) and ("Высокий" in text or "Наивысший" in text):
                ...
                # Ответ-реплай в чат с повышением приоритета
                await update.message.reply_text(
                    f"Приоритет инцидента {incident_id} ({group_name}) повышен."
                )
                ...
                if elapsed >= max_duration:
                    print(f"⚠️ С момента создания инцидента прошло больше 60 минут, напоминания не ставим.")
                    return

                seconds_50 = max(0, 50*60 - elapsed.total_seconds())
                seconds_60 = max(0, 60*60 - elapsed.total_seconds())
                ...
                if seconds_50 == 0:
                    ...
                    job_50 = scheduler.add_job(
                        notify_50_minutes,
                        trigger='date',
                        run_date=now + datetime.timedelta(seconds=seconds_50),
                        args=[context.application, incident_id],
                        id=f"{incident_id}_50"
                    )
                    incident["jobs"].append(job_50.id)
                ...
                if seconds_60 == 0:
                    ...
                    job_60 = scheduler.add_job(
                        notify_60_minutes,
                        trigger='date',
                        run_date=now + datetime.timedelta(seconds=seconds_60),
                        args=[context.application, incident_id],
                        id=f"{incident_id}_60"
                    )
                    incident["jobs"].append(job_60.id)

                print("✅ Напоминания установлены / обновлены после повышения приоритета.")
